...
|cp_gpr_hazard|4|Integer register hazard detection: RAW, WAR, WAW, and no hazard.
...
[[cp-gpr-hazard]]
==== Integer Register Hazard Coverpoint

The `cp_gpr_hazard` coverpoint checks integer register data dependencies between a producer instruction and a later consumer instruction.  Phase 1 uses a Category x Category approach: producer instructions are selected from the integer instructions that write an architectural GPR, and consumer instructions are selected from the integer instructions that read one or more architectural GPR source operands.  The generated tests cover the relevant producer/consumer category pairs rather than attempting to enumerate every possible instruction pair manually.

RAW hazard tests use a Depth=1 3-Class Filler Strategy.  Each generated RAW sequence contains exactly one intervening instruction between the producer and consumer:

* Class A: NOP filler, which preserves the dependency while adding one cycle of separation.
* Class B: Stressor filler, which performs independent integer work without reading or writing the producer destination register.
* Class C: Branch filler, which inserts an independent control-flow instruction between the producer and consumer.

[cols="1a,1a", options=header]
|===
|Generated assembly sequence
|Depth=1 SystemVerilog measurement

|
[source,asm]
----
# Instruction N-2: Producer
add x3, x1, x2

# Instruction N-1: Filler, selected from Class A/B/C
nop

# Instruction N: Consumer
add x4, x3, x1
----
|
...
The coverage expression deliberately compares the current instruction against `traceDataQ[ins.hart][ins.issue][2]`, the retired instruction two trace entries earlier.  This skips the intervening filler instruction at `traceDataQ[ins.hart][ins.issue][1]` and therefore measures the same Depth=1 producer/filler/consumer sequence emitted by the Python generator.
