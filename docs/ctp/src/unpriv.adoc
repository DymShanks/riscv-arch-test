...
|cp_gpr_hazard|1-3|Integer register hazard detection.
...
Depth=_n_ means there are _n_ instructions between the producer and consumer.
Depth=0 means the consumer immediately follows the producer.
Depth=1 means exactly one filler instruction separates them.

The `cp_gpr_hazard` coverpoint checks integer register data dependencies
between a producer and a consumer instruction using a Category x Category
approach. The generating producer instruction is always `add`. Consumer
instructions vary by category.

RAW hazard tests are generated at Depth=0 and Depth=1. Depth=1 tests use two filler classes:

* Class A: NOP filler (`addi x0, x0, 0`).
* Class B: Structural stressor filler — independent integer work on the same
  functional unit, without touching the producer destination register.

WAW (Write-After-Write) hazard tests are generated at Depth=0, where both
producer and consumer write the same destination register.
The consumer result must win.
