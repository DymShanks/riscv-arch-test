...
def _make_gpr_hazard(
    ...
    bin_name = case_idx if isinstance(case_idx, str) else (haz_type if field is None else f"{haz_type}_{field}_{case_idx}")
    ...
    if "r" in haz_class:
        ...
        for field in source_fields:
            for filler_name, filler in FILLERS.items():
                ...
                test_lines.extend(
                    _make_gpr_hazard(instr_name, instr_type, coverpoint, test_data, "raw", field, bin_name, filler, filler_name)
